#!/usr/bin/env python3
"""
Test Independent Multi-Agent with Majority Voting on MedQA questions.
...
3. Majority voting:
   - If 2+ agents agree: use majority (0 additional LLM calls)
   - If no majority: LLM synthesis (1 additional LLM call)

Total: 4-5 LLM calls per question (5 only when there's no consensus)
...
from src.baselines.independent_multi_agent_majority import run_independent_multi_agent_majority
...
def test_independent_multi_agent_majority(
    n_samples: int = 10,
    config_path: str = "configs/qwen25_32b_temp00.yaml",
    dataset_path: str = None,
    output_dir: str = "runs/independent_multi_agent_majority"
):
    """
    Test independent multi-agent with majority voting on N questions.
    """

    console.print("\\n[bold cyan]=" * 60)
    console.print("[bold cyan]Independent Multi-Agent (Majority Voting) - Test")
    ...
    # Track statistics
    n_majority = 0  # How many times majority voting was used
    n_synthesis = 0  # How many times synthesis was needed
    ...
    with Progress(
        ...
        task = progress.add_task(
            "[cyan]Evaluating Independent Multi-Agent (Majority)...",
            total=len(dataset)
        )
        ...
        for i, item in enumerate(dataset, 1):
            ...
            try:
                ...
                result = run_independent_multi_agent_majority(question, options, llm_client, config)

                answer = result["answer"]
                specialists = result.get("specialists", [])
                agent_responses = result.get("agent_responses", [])
                decision_method = result.get("decision_method", "unknown")
                tokens = result.get("tokens_used", 0)
                latency = result.get("latency_seconds", 0)
                num_llm_calls = result.get("num_llm_calls", 0)

                # Track decision method
                if decision_method == "majority":
                    n_majority += 1
                elif decision_method == "synthesis":
                    n_synthesis += 1
                ...
                console.print(f"  Decision: {decision_method.upper()} ({num_llm_calls} LLM calls)")
    ...
    avg_llm_calls = sum(r.get("num_llm_calls", 0) for r in valid_results) / n_total if n_total > 0 else 0
    ...
    summary = {
        ...
        "n_correct": n_correct,
        "accuracy": accuracy,
        "n_majority_used": n_majority,
        "n_synthesis_used": n_synthesis,
        "pct_majority": n_majority / n_total if n_total > 0 else 0,
    ...
    console.print(f"\\nDecision Methods:")
    console.print(f"  Majority Voting: {n_majority} ({n_majority/n_total:.1%})")
    console.print(f"  LLM Synthesis: {n_synthesis} ({n_synthesis/n_total:.1%})")
    console.print(f"\\nEfficiency:")
    console.print(f"  Avg LLM calls: {avg_llm_calls:.2f}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Test Independent Multi-Agent with Majority Voting")
    ...
    parser.add_argument("--output", type=str, default="runs/independent_multi_agent_majority", help="Output directory")

    args = parser.parse_args()

    test_independent_multi_agent_majority(
        n_samples=args.n,

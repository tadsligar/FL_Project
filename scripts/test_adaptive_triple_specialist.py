#!/usr/bin/env python3
"""
Test Adaptive Triple Specialist Baseline on MedQA questions.

Two-step approach:
1. LLM selects top 3 specialists for the question
2. LLM answers as triple board-certified in those areas

Efficiency: 2 LLM calls per question (vs 1 for zero-shot, 7 for physician debate)
"""
...
from src.baselines.adaptive_triple_specialist import run_adaptive_triple_specialist
...
def test_adaptive_triple_specialist(
    n_samples: int = 10,
    config_path: str = "configs/qwen25_32b_temp00.yaml",
    dataset_path: str = None,
    output_dir: str = "runs/adaptive_triple_specialist"
):
    """
    Test adaptive triple specialist baseline on N questions.
    """

    console.print("\n[bold cyan]=" * 60)
    console.print("[bold cyan]Adaptive Triple Specialist Baseline - Test")
    ...
    with Progress(
        ...
        task = progress.add_task(
            "[cyan]Evaluating Adaptive Triple Specialist...",
            total=len(dataset)
        )
        ...
        for i, item in enumerate(dataset, 1):
            ...
            try:
                ...
                result = run_adaptive_triple_specialist(question, options, llm_client, config)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Test Adaptive Triple Specialist Baseline")
    ...
    parser.add_argument("--output", type=str, default="runs/adaptive_triple_specialist", help="Output directory")

    args = parser.parse_args()

    test_adaptive_triple_specialist(
        n_samples=args.n,

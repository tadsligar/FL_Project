#!/usr/bin/env python3
"""
Test Debate++ (Confidence-Weighted Debate) on MedQA questions.
"""
...
from src.baselines.debate_plus import run_debate_plus
...
def test_debate_plus(
    n_samples: int = 10,
    config_path: str = "configs/qwen25_32b.yaml",
    dataset_path: str = None,
    output_dir: str = "runs/debate_plus_test",
    rounds: int = 2
):
    """
    Test Debate++ (Confidence-Weighted) on N questions.
    """

    console.print("\\n[bold cyan]=" * 60)
    console.print("[bold cyan]Debate++ (Confidence-Weighted) - Test")
    ...
    with Progress(
        ...
        for i, item in enumerate(dataset, 1):
            ...
            try:
                ...
                confidence_scores = result.get("final_confidence_scores", {})
                ...
                results.append({
                    ...
                    "tokens": tokens,
                    "latency": latency,
                    "confidence_agent_a": confidence_scores.get("agent_a", "UNKNOWN"),
                    "confidence_agent_b": confidence_scores.get("agent_b", "UNKNOWN"),
                    "both_low_confidence": confidence_scores.get("both_low", False),
                    "full_result": result
                })

                status = "[OK]" if is_correct else "[X]"
                console.print(f"  {status} Answer: {answer} ({latency:.1f}s, {tokens} tokens)")
                console.print(f"  Confidence: A={confidence_scores.get('agent_a', '?')} B={confidence_scores.get('agent_b', '?')}")
    ...
    # Confidence analysis
    n_both_low = sum(1 for r in valid_results if r.get("both_low_confidence"))
    ...
    summary = {
        ...
        "n_both_low_confidence": n_both_low,
        "config": {
            "model": config.model,
    ...
    console.print(f"Both Agents LOW Confidence: {n_both_low} questions")
    console.print(f"\nResults saved to: [cyan]{output_path}[/cyan]\n")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Test Debate++ (Confidence-Weighted)")
    ...
    parser.add_argument("--output", type=str, default="runs/debate_plus_test", help="Output directory")

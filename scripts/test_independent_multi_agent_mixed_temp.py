#!/usr/bin/env python3
"""
Test Independent Multi-Agent with Mixed Temperatures on MedQA questions.

Architecture:
1. LLM selects top 3 specialists for the question (1 LLM call, temp=0.0)
2. Each specialist independently analyzes the question (3 LLM calls, temp=0.3)
3. Final reviewer synthesizes responses and makes choice (1 LLM call, temp=0.0)

Total: 5 LLM calls per question
Temperature: 0.0 for selector/reviewer, 0.3 for specialists
"""
...
from src.baselines.independent_multi_agent_mixed_temp import run_independent_multi_agent_mixed_temp
...
def test_independent_multi_agent_mixed_temp(
    n_samples: int = 10,
    config_path: str = "configs/qwen25_32b_temp00.yaml",
    dataset_path: str = None,
    output_dir: str = "runs/independent_multi_agent_mixed_temp",
    ...
    """
    Test independent multi-agent with mixed temperatures on N questions.
    """

    console.print("\n[bold cyan]=" * 60)
    console.print("[bold cyan]Independent Multi-Agent (Mixed Temp) - Test")
    ...
    console.print(f"  Base Temperature: {config.temperature}")
    console.print(f"  Selector Temp: {selector_temp}")
    console.print(f"  Specialist Temp: {specialist_temp}")
    console.print(f"  Reviewer Temp: {reviewer_temp}\n")
    ...
    with Progress(
        ...
        task = progress.add_task(
            "[cyan]Evaluating Independent Multi-Agent (Mixed Temp)...",
            total=len(dataset)
        )
        ...
        for i, item in enumerate(dataset, 1):
            ...
            try:
                ...
                result = run_independent_multi_agent_mixed_temp(
                    question, options, llm_client, config,
                    selector_temp=selector_temp,
                    specialist_temp=specialist_temp,
                    reviewer_temp=reviewer_temp
                )
    ...
    console.print(f"\nTemperatures:")
    console.print(f"  Selector: {selector_temp}")
    console.print(f"  Specialist: {specialist_temp}")
    console.print(f"  Reviewer: {reviewer_temp}")
    console.print(f"\nEfficiency:")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Test Independent Multi-Agent with Mixed Temperatures")
    ...
    parser.add_argument("--output", type=str, default="runs/independent_multi_agent_mixed_temp", help="Output directory")
    parser.add_argument("--selector-temp", type=float, default=0.0, help="Temperature for selector agent")
    parser.add_argument("--specialist-temp", type=float, default=0.3, help="Temperature for specialist agents")
    parser.add_argument("--reviewer-temp", type=float, default=0.0, help="Temperature for reviewer agent")
    parser.add_argument("--resume", type=str, default=None, help="Resume from existing run directory (full path)")

    args = parser.parse_args()

    test_independent_multi_agent_mixed_temp(
        n_samples=args.n,
        config_path=args.config,
        dataset_path=args.dataset,
        output_dir=args.output,
        selector_temp=args.selector_temp,
        specialist_temp=args.specialist_temp,
        reviewer_temp=args.reviewer_temp,

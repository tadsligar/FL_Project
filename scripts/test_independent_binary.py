#!/usr/bin/env python3
"""
Test Independent Binary Agents on MedQA questions.
"""
...
from src.baselines.independent_binary_agents import run_independent_binary_agents
...
def test_independent_binary(
    ...
    output_dir: str = "runs/independent_binary_test"
):
    """
    Test Independent Binary Agents on N questions.
    """

    console.print("\\n[bold cyan]=" * 60)
    console.print("[bold cyan]Independent Binary Agents - Test")
    ...
    with Progress(
        ...
        task = progress.add_task(
            "[cyan]Evaluating Independent Binary Agents...",
            total=len(dataset)
        )
        ...
        for i, item in enumerate(dataset, 1):
            ...
            try:
                ...
                result = run_independent_binary_agents(
                ...
                aggregation_needed = result.get("aggregation_needed", False)
                ...
                console.print(f"  {status} Answer: {answer} ({yes_count} YES votes, aggregation: {aggregation_needed}) ({latency:.1f}s, {tokens} tokens)")
    ...
    n_aggregation_needed = sum(1 for r in valid_results if r.get("aggregation_needed"))
    avg_yes_count = sum(r.get("yes_count", 0) for r in valid_results) / n_total if n_total > 0 else 0
    ...
    summary = {
        ...
        "aggregation_needed_count": n_aggregation_needed,
        "aggregation_needed_pct": n_aggregation_needed / n_total if n_total > 0 else 0,
        "avg_yes_count": avg_yes_count,
        "config": {
            "model": config.model,
    ...
    console.print(f"Avg YES votes per question: {avg_yes_count:.2f}")
    console.print(f"Aggregation needed: {n_aggregation_needed}/{n_total} = {n_aggregation_needed/n_total:.1%}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Test Independent Binary Agents")
    ...
    parser.add_argument("--output", type=str, default="runs/independent_binary_test", help="Output directory")

    args = parser.parse_args()

    test_independent_binary(
        n_samples=args.n,

#!/usr/bin/env python3
"""
Test Zero-Shot Physician Baseline on MedQA questions.

Same as zero-shot baseline but with "You are an experienced physician." added.
Tests if simple role assignment improves performance.
"""
...
from src.baselines.zero_shot_physician import run_zero_shot_physician
...
def test_zero_shot_physician(
    n_samples: int = 10,
    config_path: str = "configs/qwen25_32b_temp00.yaml",
    dataset_path: str = None,
    output_dir: str = "runs/zero_shot_physician"
):
    """
    Test zero-shot physician baseline on N questions.
    """

    console.print("\n[bold cyan]=" * 60)
    console.print("[bold cyan]Zero-Shot Physician Baseline - Test")
    ...
    with Progress(
        ...
        task = progress.add_task(
            "[cyan]Evaluating Zero-Shot Physician Baseline...",
            total=len(dataset)
        )
        ...
        for i, item in enumerate(dataset, 1):
            ...
            try:
                ...
                result = run_zero_shot_physician(question, options, llm_client, config)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Test Zero-Shot Physician Baseline")
    ...
    parser.add_argument("--output", type=str, default="runs/zero_shot_physician", help="Output directory")

    args = parser.parse_args()

    test_zero_shot_physician(
        n_samples=args.n,

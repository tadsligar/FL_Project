#!/usr/bin/env python3
"""
Test Answer Space Consultation on 10 questions.
"""
...
from src.baselines.answer_space_consultation import run_answer_space_consultation
...
def test_answer_space_consultation(
    n_samples: int = 10,
    config_path: str = "configs/qwen25_32b.yaml",
    dataset_path: str = None,
    output_dir: str = "runs/answer_space_consultation_test"
):
    """
    Test Answer Space Consultation on N questions.
    """

    console.print("\n[bold cyan]=" * 60)
    console.print("[bold cyan]Answer Space Consultation - Test")
    ...
    with Progress(
        ...
        task = progress.add_task(
            "[cyan]Evaluating Answer Space Consultation...",
            total=len(dataset)
        )
        ...
        for i, item in enumerate(dataset, 1):
            ...
            try:
                ...
                result = run_answer_space_consultation(
                ...
                results.append({
                    ...
                    "specialists": result.get("answer_space_analysis", {}).get("specialists", []),
                    "full_result": result
                })

                status = "[OK]" if is_correct else "[X]"
                console.print(f"  {status} Answer: {answer} ({latency:.1f}s, {tokens} tokens)")
                console.print(f"  Specialists: {', '.join(result.get('answer_space_analysis', {}).get('specialists', []))}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Test Answer Space Consultation")
    ...
    parser.add_argument("--output", type=str, default="runs/answer_space_consultation_test", help="Output directory")

    args = parser.parse_args()

    test_answer_space_consultation(
        n_samples=args.n,
